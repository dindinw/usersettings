...
void Usage() {
    //if (sscanf(inputstring, "%9s %hu/%hu/%hu %hu:%hu:%hu %2s", day, &systemtime->wMonth, &systemtime->wDay, &systemtime->wYear, &systemtime->wHour, &systemtime->wMinute, &systemtime->wSecond, daynight) == 0) {
    printf("timestomp filename -m time -a time -c time -e time\n"
        "time is of format : dayOfWeek mm/dd/yy hour minute second am/pm"
        );
    exit(1);
}
...
void PrintSystemTime(SYSTEMTIME systime) {
    printf("%2d/%2d/%2d %2d:%2d:%2d dayOfWeek:%d",systime.wMonth,systime.wDay,systime.wYear,systime.wHour,systime.wMinute,systime.wSecond,systime.wDayOfWeek);
}
...
int main(int argc,char**argv) {
    ...
    filename[63] = '\0';
    FILE_BASIC_INFORMATION fbi;
    HANDLE file = RetrieveFileBasicInformation(filename,&fbi);
    if(file==0) {
        error_exit("can not open file");
    }
    SYSTEMTIME systime;
    ConvertLargeIntegerToLocalTime(&systime,fbi.LastWriteTime);
    printf("\nm:");
    PrintSystemTime(systime);
    ConvertLargeIntegerToLocalTime(&systime,fbi.LastAccessTime);
    printf("\na:");
    PrintSystemTime(systime);
    ConvertLargeIntegerToLocalTime(&systime,fbi.CreationTime);
    printf("\nc:");
    PrintSystemTime(systime);
    ConvertLargeIntegerToLocalTime(&systime,fbi.ChangeTime);
    printf("\ne:");
    PrintSystemTime(systime);
    systime.wYear-=10;
    ConvertLocalTimeToLargeInteger(systime,&fbi.ChangeTime);
    SetFileMACE(file,fbi);
}
